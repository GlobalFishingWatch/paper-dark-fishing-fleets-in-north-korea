...
from .pair_trawler_tile_data import NoisyTrainTestTileData as TrainTestTileData
...
def model_fn(features, labels, mode, params):
    ...
    def make_output(n, Y=Ycommon, count=384):
        Y = layer(Y, 5, count, 'conv1')
        Y = layer(Y, 5, count, 'conv2')
        Y = layer(Y, 1, count, 'conv3')
        Y = layer(Y, 1, n,     'conv4', nonlin=None)
        return Y

    with tf.variable_scope('Yisship'):
        # Give is_ship more filters because it is most important to get right
        Yship_logits = make_output(1, count=768)
